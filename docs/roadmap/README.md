# Roadmap

Per-category files in this folder. Items inside each are grouped by
priority (Now / Next / Later / Speculative).

## Now

- [Scroll](scroll.md) — drag-to-pan thumb, `scroll_to`
- [Layering](layering.md) — overlay / popup layer
- [Accessibility](accessibility.md) — accesskit integration

## Categories

- [Layouts](layouts.md) — stack / grid / universal (flex-basis, baseline, percent, named areas, position)
- [TextEdit](text-edit.md) — selection, IME, clipboard, undo, multi-line
- [Focus](focus.md) — tab cycling, focus ring, restoration
- [Scroll](scroll.md) — scrollbar UX, virtualization, inertia
- [Damage](damage.md) — multi-rect, debug overlay, encode replay
- [Invalidation](invalidation.md) — property tracker, discard
- [Caches](caches.md) — intrinsic, subtree encode, hit-hint
- [Renderer / GPU](renderer.md) — staging belt, offscreen, nested clips
- [Text](text.md) — cache eviction, atlas, allocs
- [Input](input.md) — coalescing, drag-and-drop
- [Layering](layering.md) — popups, z-order, multi-window
- [Accessibility](accessibility.md) — accesskit
- [Tooling](tooling.md) — profiling, snapshots, HiDPI
- [i18n](i18n.md) — RTL, BiDi
- [Speculative](speculative.md) — profile-gated micro-optimizations
- [Scratch](scratch.md) — unsorted ideas

## Sibling docs

- [Popups](../popups.md)
- [WidgetId trait](../widget-id-trait.md)
