...
1. **Composer cache.** Biggest win available. The composer is now the dominant constant in `end_frame` once the encode cache is in (it walks every cmd → quad even when the encoder hits 100 %). Plan in `docs/composer-cache.md` — cascade-keyed variant (A), 3 steps (~2 days), gated on a Step-1 cold-frame spike. Subtree-marker mechanism decided: in-stream `EnterSubtree/ExitSubtree` cmds.
