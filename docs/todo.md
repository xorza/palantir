...
1. **Composer cache.** Biggest win available. The composer is now the dominant constant in `end_frame` once the encode cache is in (it walks every cmd → quad even when the encoder hits 100 %). Same structural pattern: snapshot `(quads, texts, groups)` per cached encode subtree, rewrite scissor rects under the current root origin at replay. Pulls nested over the 20 % goal naturally.
