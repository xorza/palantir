...
1. **Composer cache.** Biggest win available. The composer is now the dominant constant in `end_frame` once the encode cache is in (it walks every cmd → quad even when the encoder hits 100 %). Design exploration in `docs/composer-cache.md` — short version: cascade-keyed variant (A) is ~300 lines mirroring the encode cache; translate-aware variant (B) is ~600 lines with snap edge cases. Recommend shipping A first.
