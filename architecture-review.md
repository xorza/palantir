...
- [x] **Internalize the renderer driver and make `lib.rs` the only supported type namespace.** Every implementation parent is now crate-private and the blanket private-interface lint suppression is gone (`src/lib.rs:8-26`); the supported `OffscreenHost`/`WinitHost` facades and consumer vocabulary are re-exported only at the root, including `MeasureResult`, `FramePaint`, and `FrameProcessing` (`src/lib.rs:63-137`). `RecordStore`, `Frontend`, `WindowRenderer`, its frame methods, `FrameTarget`, and `FramePresent` are internal driver details (`src/record_store.rs:57`; `src/renderer/frontend/mod.rs:31-43`; `src/host/window_renderer.rs:33-48,284-393,598-631`). `RenderPlan`/`RenderKind` and the detailed damage region stay crate-private while `FrameReport::paint` exposes the stable skip/full/partial classification, with exact coverage of every plan shape (`src/ui/frame_report.rs:15-60,82-165`). Internals-gated external consumers now use narrow root helpers rather than nested implementation paths (`src/lib.rs:56-61`; `benches/frame.rs:54,194-232`).
