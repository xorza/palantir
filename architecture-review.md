...
- [x] **Internalize the renderer driver and make `lib.rs` the only supported type namespace.** Every production implementation parent is crate-private and the blanket private-interface lint suppression is gone (`src/lib.rs:8-28`); supported `OffscreenHost`/`WinitHost` facades and consumer vocabulary are re-exported only at the root (`src/lib.rs:30-151`). `RecordStore`, `Frontend`, `WindowRenderer`, its frame methods, `FrameTarget`, and `FramePresent` remain internal driver details (`src/record_store.rs:57`; `src/renderer/frontend/mod.rs:31-68`; `src/host/window_renderer.rs:33-48,284-393,598-631`). `RenderPlan`/`RenderKind` and the detailed damage region stay crate-private while `FrameReport::paint` exposes the stable skip/full/partial classification, with exact coverage of every plan shape (`src/ui/frame_report.rs:15-60,82-165`). The only feature-gated external surfaces are the dedicated benchmark namespace and narrow integration-test helpers; neither publishes production implementation types (`src/lib.rs:8-10,30-31`; `src/bench/mod.rs:1-24`).

- [x] **Centralize benchmarks behind a narrow `internals` facade before sealing modules.** Every Criterion driver now lives under `src/bench/` in a hierarchy mirroring the production subsystem: damage/cascade, text atlas/shape, layout caches, input, and composer call crate-private production types directly from `src/bench/{ui,renderer,text,layout,input}`. The cross-subsystem frame driver and shared allocation fixture have their own neutral `src/bench/frame/` hierarchy (`src/bench/frame/mod.rs:1-63`; `src/bench/frame/fixture.rs:1-109`). One `internals`-gated facade exports Criterion entry functions plus the benchmark-owned `FrameFixture`, with no production implementation structs (`src/lib.rs:8-10`; `src/bench/mod.rs:7-24`). Criterion targets are five-line wiring callers except the nine-line custom-config frame target (`benches/caches.rs`; `benches/damage.rs`; `benches/frame.rs`); only the three `dhat` targets retain their required allocator/measurement shells externally.
