...
```text
window A FullRecord: tree A stores spans into shared FrameArena(A)
window B FullRecord: clears the same arena and writes FrameArena(B)
window A PaintOnly:  retains tree A, skips record, reads its spans from FrameArena(B)
```
...
## Batch 1 — Critical: make frame geometry truly per-window

- [x] **Give every `WindowRenderer` its own `FrameArena` and pass it explicitly to GPU submission.** `HostContext` currently owns one arena and clones it into every `Ui` (`src/host/context.rs:27-35`, `src/ui/mod.rs:151-165`) and the one backend (`src/renderer/backend/mod.rs:167-172,205-217`). `Ui::record_pass` clears it (`src/ui/mod.rs:451-459`), while `PaintOnly` deliberately retains the old tree and arena indices (`src/ui/mod.rs:184-195,219-230`). Serialized record-to-submit execution does not protect retained indices between different windows' frames. Reserve a fresh arena in `WindowRendererBuilder::build`, give that handle to the window's `Ui`/frontend, and add `arena: &FrameArenaInner` (or `&FrameArena`) to `Submission` instead of storing an arena on `WgpuBackend`. Update the misleading ownership docs in `frame_arena.rs` and `host/context.rs`. Verify with two windows containing different mesh/polyline/text payload lengths: record A, record B, then drive an animation-only paint on A and assert A's geometry/pixels remain unchanged.

## Batch 2 — Critical: stop retaining shared-cache physical identities

- [ ] **Do not store an evictable gradient atlas row as retained scene identity.** `LoweredGradient` captures `LutRow` in the retained arena (`src/frame_arena.rs:30-37,74-80`), but the shared atlas reuses rows after 255 live gradients (`src/renderer/gradient_atlas/mod.rs:50-68,177-205`). The epoch protection covers rows registered during the current submit only (`src/renderer/gradient_atlas/mod.rs:87-103,234-267`); a `PaintOnly` window does not re-lower/re-register its gradients. Retain a gradient definition/content handle and resolve it to a row on every frontend build or submit, including `PaintOnly`, or make the CPU/GPU atlas owner-scoped. Add an interleaved two-window test that fills the atlas, evicts A's row from B, then paints A without recording.

- [ ] **Owner-scope the `TextShaper` reuse key and removal sweep.** The shaper is shared by all windows, but reuse is keyed only by `(WidgetId, ordinal)` (`src/text/mod.rs:100-111,144-155,220-260`). Different windows routinely generate the same IDs, and one window's `sweep_removed` deletes matching rows from every window (`src/text/mod.rs:492-504`). Besides cache thrash, this can unpin a retained `PaintOnly` key; after cosmic cache eviction the backend's required lookup panics (`src/renderer/backend/text/mod.rs:306-310`). Reserve a window/render owner before constructing `Ui` and `Frontend`, include it in reuse keys and sweeps, and use the same owner throughout the frame. `RenderOwnerId` already provides the vocabulary (`src/renderer/render_buffer/owner.rs:5-12`) but is currently reserved inside `Frontend::new`, too late for `Ui`. Test two windows with the same widget ID, different text, removal in one window, and an over-budget cosmic cache.

## Batch 3 — High: input and scroll routing correctness

- [ ] **Discard events that have no current route instead of leaving them for a future widget.** Scroll/zoom accumulators are updated before checking for a target/subscriber (`src/input/mod.rs:645-666`), and key/text events are queued before the wake predicate (`src/input/mod.rs:668-699`). When `on_input` returns `requests_repaint = false`, no frame is guaranteed to drain them. A later target/focus change can therefore deliver stale input to a widget that was not targeted when the event occurred; `frame_had_action` is also set unconditionally at `src/input/mod.rs:503-518`. Determine observability first, enqueue/update only when a current target or subscriber exists, and set the action flag only for observable actions. Add sequence tests for inert-area scroll followed by moving over `Scroll`, and unfocused key/text followed by focusing `TextEdit`.

- [ ] **Only claim `Sense::PINCH` for zoom-enabled scrolls.** Every `Scroll` currently advertises `SCROLL | PINCH` (`src/widgets/scroll/mod.rs:393-405`), but zoom application is optional (`src/widgets/scroll/mod.rs:454-467,570-582`). A non-zoom inner scroll can win hit testing and discard a pinch that a zoomable ancestor should receive. Constructors should claim only `SCROLL`; `with_zoom`/`with_zoom_config` should add `PINCH`. Cover a non-zoom inner scroll nested in a zoomable outer scroll.
