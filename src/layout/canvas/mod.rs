...
/// `max(child_pos + child_desired)` per axis. Negative positions render
/// outside the canvas's `inner` (and would be clipped by any
/// surrounding scroll's viewport) — if you need scrollable negative-
/// origin canvases, see [`crate::widgets::scroll::Scroll::anchor_canvas_origin`]
/// for the userspace pattern (shift positions into positive space and
