...
fn canvas_places_child_at_position_within_inner_rect() {
    ...
    let panel_rect = ui.layout.result[Layer::Main].rect[panel.index()];
    let kids: Vec<_> = ui
        .forest
        .tree(Layer::Main)
        .children(panel)
...
fn canvas_hugs_to_bounding_box_of_placed_children() {
    ...
    let r = ui.layout.result[Layer::Main].rect[panel.index()];
...
fn canvas_negative_position_does_not_extend_bbox() {
    ...
    let kids: Vec<_> = ui
        ...
        .map(|c| c.id)
        .collect();
    let child = ui.layout.result[Layer::Main].rect[kids[0].index()];
...
fn canvas_fill_child_uses_inner_when_constrained_else_intrinsic() {
    ...
    for (label, fixed_size, expected) in cases {
        ...
        let kids: Vec<_> = ui
            ...
            .map(|c| c.id)
            .collect();
        let f = ui.layout.result[Layer::Main].rect[kids[0].index()];
...
fn canvas_ignores_child_align() {
    ...
    let r = ui.layout.result[Layer::Main].rect[child.unwrap().index()];
