...
fn vstack_with_fill_distributes_remainder() {
    ...
    let kids: Vec<_> = ui.tree.children(root).collect();
    let fixed = ui.layout_engine.rect(kids[0]);
...
fn hstack_justify_center_centers_content_block() {
    ...
    assert_eq!(ui.layout_engine.rect(kids[0]).min.x, 60.0);
    assert_eq!(ui.layout_engine.rect(kids[1]).min.x, 100.0);
}
...
fn hstack_justify_end_packs_to_trailing_edge() {
    ...
    assert_eq!(ui.layout_engine.rect(kids[0]).min.x, 120.0);
    assert_eq!(ui.layout_engine.rect(kids[1]).min.x, 160.0);
}
...
fn hstack_justify_space_between_distributes_leftover_between() {
    ...
    assert_eq!(ui.layout_engine.rect(kids[0]).min.x, 0.0);
    assert_eq!(ui.layout_engine.rect(kids[1]).min.x, 80.0);
    assert_eq!(ui.layout_engine.rect(kids[2]).min.x, 160.0);
}
...
fn hstack_justify_space_around_distributes_with_half_pads() {
    ...
    assert_eq!(ui.layout_engine.rect(kids[0]).min.x, 30.0);
    // 30 + 40 + 60 = 130
    assert_eq!(ui.layout_engine.rect(kids[1]).min.x, 130.0);
}
...
fn hstack_justify_is_noop_when_fill_child_consumes_leftover() {
    ...
    assert_eq!(ui.layout_engine.rect(kids[0]).min.x, 0.0);
    assert_eq!(ui.layout_engine.rect(kids[1]).min.x, 40.0);
    assert_eq!(ui.layout_engine.rect(kids[1]).size.w, 120.0);
    assert_eq!(ui.layout_engine.rect(kids[2]).min.x, 160.0);
}
...
fn hstack_gap_inserts_space_between_children() {
    ...
    let kids: Vec<_> = ui.tree.children(root).collect();
    assert_eq!(ui.layout_engine.rect(kids[0]).min.x, 0.0);
    assert_eq!(ui.layout_engine.rect(kids[1]).min.x, 50.0);
    assert_eq!(ui.layout_engine.rect(kids[2]).min.x, 100.0);
}
...
fn negative_left_margin_spills_outside_slot() {
    ...
    let r = ui.layout_engine.rect(button_node.unwrap());
