...
use palantir::{Color, Configure, Panel, Scroll, Sizing, SplitHalf, Splitter, Text, Ui, WidgetId};
...
fn row(ui: &mut Ui, i: u32) {
    Panel::hstack()
        ...
        .show(ui, |ui| {
            let label = ui.fmt(format_args!("row {i:02}"));
            Text::new(label)
...
fn col(ui: &mut Ui, i: u32) {
    Panel::vstack()
        ...
        .background(swatch_bg(ramp(i)))
        .show(ui, |ui| {
            let label = ui.fmt(format_args!("col {i:02}"));
            Text::new(label)
