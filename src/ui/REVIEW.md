...
### A3. `Damage::dirty` is test-only state on a production struct — REJECTED
`damage/mod.rs:60-62`, `124-157`. Field is `#[cfg(test)]`-gated with matching `#[cfg(test)]` populate branch and a `#[cfg(not(test))] let _ = dirty;` warning-silencer. Violates the "no test-only code on production types" rule on its face, but kept deliberately: multi-rect damage is on the roadmap and will need exactly this per-node identity list as production state. Removing now then re-adding later costs more than the cfg branch costs today. Revisit when multi-rect lands.
