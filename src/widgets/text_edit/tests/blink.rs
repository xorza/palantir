...
fn caret_blinks_on_and_off_while_focused() {
    ...
    fn body(ui: &mut UiCore, buf: &mut String, leaf: &mut Option<NodeId>) {
    ...
    fn caret_painted(ui: &UiCore, leaf: NodeId) -> bool {
    ...
    fn frame_at(ui: &mut UiCore, now_secs: f32, mut f: impl FnMut(&mut UiCore)) {
...
fn caret_anim_does_not_damage_between_quantum_boundaries() {
    ...
    fn record(ui: &mut UiCore, buf: &mut String) {
        Panel::hstack().auto_id().show(ui, |ui| {
            TextEdit::new(buf)
                .id_salt("anim-damage")
                .size((Sizing::Fixed(180.0), Sizing::Fixed(40.0)))
    ...
    let frame = |ui: &mut UiCore, buf: &mut String, t_secs: f32| -> FrameReport {
        let report = ui.frame(
...
fn focus_gain_resets_blink_even_without_caret_change() {
    ...
    let frame = |ui: &mut UiCore, buf: &mut String, t: f32| {
        let r = ui.frame(
