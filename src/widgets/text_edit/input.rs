...
/// `ui.shared.render.text` here, but never the shape/tree storage.
pub(crate) fn handle_input(
    ...
    // `Editor`). `ui.state`, `ui.input`, and `ui.shared.render.text` are
        ...
        && let Some(pointer_offset) = resp_state.pointer_local
    {
        ...
        let hit = ui.shared.render.text.byte_at_xy(
            ed.text,
            local_x,
    ...
    // `ui.shared.render.text` inside the same loop without a scratch Vec.
            ...
            KeyboardEvent::Down(kp) => {
                ...
                match ed.apply_key(kp) {
                    ...
                    KeyOutcome::Vertical { up, extend } => {
                        ed.resolve_vertical(&ui.shared.render.text, ctx.params(), up, extend);
                    }
                    KeyOutcome::None => {}
                }
            }
