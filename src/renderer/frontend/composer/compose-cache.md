...
- `src/renderer/frontend/composer/tests.rs::cache_integration` —
  ...
  byte-identical to a fresh cold compose; a `internals::clear_compose_cache()`
...
warm-cache frame with `internals::clear_compose_cache()` between iterations
...
cascade fingerprint quantization) live in `docs/roadmap/`.
