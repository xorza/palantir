...
/// Per-frame inputs to the pure compose pass. No GPU handles — `compose`
/// only reads commands and writes into a `RenderBuffer`.
...
/// groups (scissor ranges) into `out`. Maintains a clip stack so nested
/// `PushClip`s intersect correctly. Pure: no device, no queue, no allocations
/// in steady state (buffers are reused).
pub fn compose(cmds: &[RenderCmd], params: &ComposeParams, out: &mut RenderBuffer) {
    let viewport_phys_f = [
