# Palantir Contributor License Agreement

Thank you for your interest in contributing to Palantir ("the Project"),
maintained by the copyright holder ("the Maintainer", currently the
individual reachable at xxorza@gmail.com).

This Contributor License Agreement ("Agreement") clarifies the intellectual
property license granted with Contributions from any person or entity to the
Maintainer. It protects you, the Maintainer, and users of the Project; it
does not change your rights to use your own Contributions for any other
purpose.

By submitting a Contribution to the Project (e.g. opening a pull request,
sending a patch, or otherwise transmitting code, documentation, or other
material), you accept and agree to the following terms.

## 1. Definitions

- **"Contribution"** means any original work of authorship, including any
  modifications or additions to an existing work, that is intentionally
  submitted by you to the Maintainer for inclusion in the Project.
- **"You"** means the individual or legal entity making the Contribution.
  If your employer has rights to intellectual property you create, you
  represent that you have received permission to make Contributions on
  behalf of that employer, or that your employer has waived such rights.

## 2. Copyright License

You hereby grant to the Maintainer, and to recipients of software
distributed by the Maintainer, a perpetual, worldwide, non-exclusive,
no-charge, royalty-free, irrevocable copyright license to reproduce,
prepare derivative works of, publicly display, publicly perform,
sublicense, and distribute your Contributions and such derivative works.

## 3. Patent License

You hereby grant to the Maintainer, and to recipients of software
distributed by the Maintainer, a perpetual, worldwide, non-exclusive,
no-charge, royalty-free, irrevocable (except as stated in this section)
patent license to make, have made, use, offer to sell, sell, import, and
otherwise transfer your Contribution, where such license applies only to
those patent claims licensable by you that are necessarily infringed by
your Contribution alone or by combination of your Contribution with the
Project.

## 4. Right to Relicense

You acknowledge and agree that the Maintainer may license the Project
(including your Contributions) under any license terms, including but not
limited to:

- The GNU General Public License version 3 or any later version, and
- Proprietary, commercial license terms negotiated by the Maintainer on a
  per-recipient basis.

This relicensing right is the basis of the Project's dual-licensing model.
You retain all rights, title, and interest in your Contributions and may
use them for any other purpose; this Agreement is a license grant to the
Maintainer, not an assignment of copyright.

## 5. Representations

You represent that:

1. Each of your Contributions is your original creation, or you have
   sufficient rights to grant the licenses in this Agreement.
2. Your Contribution submissions include complete details of any
   third-party license or other restriction (including, but not limited to,
   related patents and trademarks) of which you are personally aware and
   which are associated with any part of your Contributions.
3. You are legally entitled to grant the above licenses. If your employer
   has rights to intellectual property that you create, you represent that
   you have received permission to make the Contributions on behalf of that
   employer, or that your employer has waived such rights.

## 6. No Warranty

Your Contributions are provided "AS IS", without warranty of any kind,
express or implied, including without limitation warranties of
merchantability, fitness for a particular purpose, or non-infringement.

## 7. Acceptance

You accept the terms of this Agreement by adding a `Signed-off-by` trailer
to each commit in a Contribution, in the form:

    Signed-off-by: Your Real Name <your.email@example.com>

Adding this trailer certifies that you have read this Agreement and agree
to be bound by its terms with respect to the Contribution(s) contained in
that commit. Git provides the `-s` / `--signoff` flag (`git commit -s`) to
add this trailer automatically.
