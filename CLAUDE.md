...
- `src/window_renderer.rs` — `WindowRenderer` pipeline owner (recorder `Ui` + `Frontend` + `WgpuBackend`) + `FramePresent` scheduling enum; public entry `WindowRenderer::frame` (swapchain); `WindowRenderer::frame_offscreen` (bench / headless render to a `wgpu::Texture`) is internals-gated in `window_renderer::test_support`
- `src/winit_host.rs` — `WinitHost<T>` + `WinitHostConfig`: winit `ApplicationHandler` glue around `WindowRenderer`. Owns window/surface, picks sRGB swapchain, maps `FramePresent` → `ControlFlow`, forwards `WindowEvent`s to `Ui::on_input`. Config exposes title, initial/min logical size, present mode, power preference. The swapchain is always double-buffered (`desired_maximum_frame_latency: 1`).
