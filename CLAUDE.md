...
- `src/forest/` — `Forest` + `Layer` enum + `CollisionRecord` list (per-layer arenas, `mod.rs`), `tree/` (per-layer `Tree`: SoA records + packed `ExtrasIdx` + dense `bounds_table`/`panel_table`/`chrome_table` (`ChromeRow` holds chrome+`ClipMode::Rounded` radius) + `Shapes` + `GridArena` + `SubtreeRollups` + `PaintAnims`, `NodeId`; `tree/iter.rs` traversal iterators (`ChildIter`/`TreeItems` — the shape-cursor source of truth the encoder/cascade/hash walks all drive), `tree/node.rs` (`NodeRecord`/`SubtreeEnd`), `tree/recording.rs` recording-only scratch (`RecordingScratch`/`RootSlot`) kept off the finalized `Tree` so downstream `&Tree` passes can't reach transient state), `element/` (Element builder, `LayoutCore`/`NodeFlags`/`LayoutMode`, `Configure`), `frame_arena.rs` (`FrameArena` — variable-length shape payloads), `per_layer.rs` (`PerLayer`), `rollups.rs` (per-node + subtree hashes), `shapes/` (`ShapeRecord` + add/clear), `seen_ids.rs`, `visibility.rs`
