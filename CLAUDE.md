...
- `src/host.rs` — `Host<T>` pipeline owner (recorder `Ui` + `Frontend` + `WgpuBackend`) + `FramePresent` scheduling enum; single public entry `Host::frame`
- `src/winit_host.rs` — `WinitHost<T,F>` + `WinitHostConfig`: winit `ApplicationHandler` glue around `Host`. Owns window/surface, picks sRGB swapchain, maps `FramePresent` → `ControlFlow`, forwards `WindowEvent`s to `Ui::on_input`. Config exposes title, initial/min logical size, present mode, power preference, frame latency.
