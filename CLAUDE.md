...
- `src/host.rs` — `Host` pipeline owner (recorder `Ui` + `Frontend` + `WgpuBackend`) + `FramePresent` scheduling enum; public entry `Host::frame` (swapchain); `Host::frame_offscreen` (bench / headless render to a `wgpu::Texture`) is internals-gated in `host::test_support`
