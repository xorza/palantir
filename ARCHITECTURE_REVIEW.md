...
- [x] **Choose one owner for stroked-shape bbox inflation.** Lowering now retains only the tight centerline AABB for polylines, curves, and arcs. The shared `stroked_bbox` raster contract applies width, square-cap projection, miter-limit reach, and the half-pixel AA fringe exactly once: cascade converts the physical fringe back through the active transform/DPI for logical damage, while composer evaluates the same formula directly in physical pixels for culling and overlap ordering. Two-point polylines omit nonexistent join reach. Exact tests cover cap/join variants, transforms, 0.5x/1x/2x display scales, rotation-invariant payload bounds, and the group-count boundary that previously flushed on a false overlap.
